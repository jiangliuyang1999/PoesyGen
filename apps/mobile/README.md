...
移动端支持主题润色、正文与格律标注查看、多项局部修改、版本切换和历史记录管理。
格律标注显示当前版本通过状态，并标记错误、存疑及超出词谱字数的字符；历史记录可
删除或进入详情查看完整词谱。
移动端词谱列表按词牌拼音排序并一次显示全部词牌；创作页词牌选项也使用相同顺序，两个
页面首次载入时均默认选择第一个词牌的第一个体式。切换页面时，字典查询、历史筛选、
选中记录和详情状态会继续保留，但页面会滚动到顶部。
...
应用不依赖 PoesyGen 服务端。词谱、韵书和字典随应用打包；用户配置自己的
