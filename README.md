...
- 创作、历史记录、词谱和字典四个独立页面；页面切换时保留各自的查询、筛选、分页、
  ...
  互不影响。
- 页面布局按可用宽度响应式切换；宽度不超过 820px 时，手机浏览器和 Capacitor 应用
  共用相同的顶部状态栏、底部导航、单列卡片及列表详情交互，不依赖移动平台判断。
- 音韵字典支持中文输入法单字查询，提交时校验输入内容，也可直接点击韵部中的汉字查询。
- 词谱例词默认显示《御定词谱》原始繁体字形，可在例词顶部切换为由 Unihan 与项目古籍
...
- 生成结果支持正文、格律标注和局部修改三种视图。局部修改可同时选择多段字、词、片段
  或整句，分别填写意见后统一重新生成。格律标注视图在版本切换按钮同一行的右侧显示
  当前版本是否通过校验，并对错误、存疑及超出词谱字数的字符进行醒目标记。
...
- 本地历史最多保存 40 条并支持搜索，宽屏 Web 和桌面端分页展示，窄屏 Web 与移动应用
  一次展示全部记录；详情突出词牌、体式、韵脚和创作主题，可从词牌卡展开完整词谱
  预览，优化轮数与附加要求以简洁文本展示，并可从记录卡片删除不再需要的记录。内部
  UUID 只用于记录与版本关联，不在界面中显示。
- 宽屏 Web 和桌面端使用左右双栏创作布局；窄屏 Web 与移动应用使用底部导航和单列
  卡片，并按“词牌设置、词牌预览、创作主题、生成设置、生成结果”排列创作内容。
...
点击页面右上角的 `LLM 未配置`，填写以下 OpenAI-compatible 参数；手机浏览器会使用
与移动应用相同的顶部配置入口：
