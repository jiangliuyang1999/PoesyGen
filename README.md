...
生成前将 `.env.example` 复制为 `.env`，至少配置 `LLM_BASE_URL`（或
`LLM_ENDPOINT`）、`LLM_MODEL` 和 `LLM_API_KEY`。CLI 会静默加载仓库根目录中存在的
`.env`，也支持直接从当前进程环境读取这些变量；最终只检查环境变量是否完整：
...
交互模式缺少必填变量时会逐项提示，并仅用于当前 CLI 进程；非交互模式仍会明确列出
缺失变量。CLI 会在当前进程调用 LLM，并运行与 Web 相同的格律校验和循环修复流程。
