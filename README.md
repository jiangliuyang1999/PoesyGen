...
LangGraph 依次调度篇章策划、初稿创作、确定性格律校验、文学评价和分类修订 Role。
...
- 可填写创作主题、最大优化轮数和附加要求，并支持调用已配置的 LLM 保持原意重新润色。
  ...
  当前任务，节点完成后输出结果，不使用与真实执行状态无关的定时心跳。
- 首次创作先把主题整理为人物、场景、情感曲线和意象边界，再按词谱生成上下阕及逐句
  写作计划；一次生成一个初稿，先通过程序格律校验，再评价主题契合、篇章连贯、情感
...
每个阶段完成后输出词谱蓝图、主题简报、篇章规划、创作初稿及每轮校验、评价和优化结果，
